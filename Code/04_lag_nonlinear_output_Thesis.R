...
# Tmin DLNM + PDSI DLNM
load("C:\\Users\\u271201\\Downloads\\output\\model1.3_NO2_interaction.RData")
model0 <- model
# load urban interaction models
load("C:\\Users\\u271201\\Downloads\\output\\model1.3_O3_interaction.RData")
model1 <- model
load("C:\\Users\\u271201\\Downloads\\output\\model1.3_PM10_interaction.RData")
model2 <- model
load("C:\\Users\\u271201\\Downloads\\output\\model1.3_PM25_interaction.RData")
model3 <- model
load("C:\\Users\\u271201\\Downloads\\output\\model0.1.RData")
model0.4 <- model
...
predt <- crosspred(basis_MaxTm, coef = coef[indt], vcov=vcov[indt,indt],
                   model.link = "log", bylag = 0.25, cen = round(mean(data$MaxTm), 0)) 

# contour and scenario plots for Tmin (Main text Fig 3)

# contour plot of exposure-lag-response associations (Main text Fig 3a)
pdf("C:\\Users\\u271201\\Downloads\\fig_03a_Tmin_contour.pdf", width = 6.5, height = 6)
...
pdf("C:\\Users\\u271201\\Downloads\\fig_03b_Tmin_scenario.pdf", width = 6, height = 6)
...
for (j in 1:length(mod.name))
{
  ...
  coef <- model0.4$summary.fixed$mean
  vcov <- model0.4$misc$lincomb.derived.covariance.matrix
  
  # create indicators for terms associated with PDSI cross-basis
  indp <- grep("basis_MaxTm",model0.4$names.fixed)
...
pdf("C:\\Users\\u271201\\Downloads\\fig_05_pdsi_scenario.pdf", width = 12, height = 6)
...
for (j in c(6:8))
{
  ...
  # extract coefficients and variance-covariance matrix
  coef <- model0.5$summary.fixed$mean
  vcov <- model0.5$misc$lincomb.derived.covariance.matrix
  
  # create indicators for terms associated with PDSI cross basis
  indp <- grep("basis_MaxTm",model0.5$names.fixed)
  
  # extract predictions from the PDSI DLNM centred on zero (normal conditions)
  predp <- crosspred(basis_MaxTm, coef = coef[indp], vcov = vcov[indp,indp],
                     model.link = "log", bylag = 0.25, cen = 30)
  ...
  tcol2 <- do.call(rgb, c(as.list(col2rgb(col2)), alpha = 255/4, max = 255))
   ...
   col3 <- brewer.pal(11, "BrBG")[8]
  tcol3 <- do.call(rgb, c(as.list(col2rgb(col3)), alpha = 255/4, max = 255))
  
  col4 <- brewer.pal(11, "BrBG")[5]
  tcol4 <- do.call(rgb, c(as.list(col2rgb(col4)), alpha = 255/4, max = 255))
  
  # define x values (lag, by lag)
  lagbylag <- seq(0, nlag, 0.25)
  
  plot(predp,"overall",xlab = expression(paste("Temperature (",degree,"C)")), 
       ylab = "Relative risk", main = "", 
       ylim = c(range(predp$allRRlow, predp$allRRhigh)))
  box()
  mtext(side = 2,text="b",cex = 1.2, las = 2, 
        at = max(predp$allRRhigh)*1.2,line = 2.5)
  
  # legend("top", legend = c(paste0("PDSI = ",vars[mn]) , paste0("PDSI = ",vars[mx])), col = c(col1, col2), pch=20, lwd = 2, lty = 1, bty = "n", y.intersp = 1.5)
  legend("top", legend = c("Extreme drought","Extremely wet", "afad", "Afaf"), col = c(col1, col2,col3, col4), lwd = 2, lty = 1, bty = "n", y.intersp = 1.5)
  mtext(side = 2, text = lab[j], cex = 1.2, las = 2, at = max(r2, 1.4)*1.1, line = 2)
}
